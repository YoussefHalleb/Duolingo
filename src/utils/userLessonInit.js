...
const initializeUserLessons = (user) => {
  if (user) {
    ...
    get(globalLessonsRef).then((snapshot) => {
      ...
      get(userLessonsRef).then((userSnapshot) => {
        if (!userSnapshot.exists()) {
          console.log("No user lessons found, initializing for user:", user.uid);
          set(userLessonsRef, globalLessons)
            .then(() => {
              console.log("Lessons successfully set:", globalLessons); // Confirmer la mise à jour
              ...
              set(userTerminatedLessonsRef, initialTerminatedLessons)
                .then(() => {
                  set(ref(rtdb, `users/${user.uid}`), {
                    ...
                    .catch((error) => console.error('Error initializing username:', error));
                })
                .catch((error) => console.error('Error initializing terminatedLessons:', error));
            })
            .catch((error) => console.error('Error initializing lessons:', error));
        } else {
          console.log("Lessons already exist for user:", user.uid);
        }
      }).catch((error) => console.error('Error checking user lessons:', error));
    }).catch((error) => console.error('Error fetching global lessons:', error));
  }
};
...
export const setupUserLessonInit = () => {
  onAuthStateChanged(auth, (user) => {
    if (user) {
      ...
      get(userLessonsRef).then((snapshot) => {
        if (!snapshot.exists()) {
      ...
      }).catch((error) => console.error('Error checking lesson initialization:', error));
    }
  });
};
